...
const updateStory = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(_id)) return res.status(404).send("This id doesnt belong to any story");
...
const deleteStory = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).send("This id doesnt belong to any story");
...
const likeStory = async (req, res) => {
    ...
    if (index === -1)  story.likes.push(req.userId);
    else story.likes = story.likes.filter(id => id !== String(req.userId));
